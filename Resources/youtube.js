...
//if anything bad happens, show the error to the user and log it
		...
		Titanium.UI.createAlertDialog({title:'', message:'No videos were found for this search.'}).show();
...
//create the YouTube API search URL from the function parameters
	var searchUrl = 'http://gdata.youtube.com/feeds/api/videos?alt=rss';

	if (channel && channel.length > 0)
           searchUrl += '&author=' + escape(channel);

	if (searchTerm && searchTerm.length > 0)
	    searchUrl += '&q=' + escape(searchTerm);

	searchUrl += "&orderby=published&max-results=25&v=2";
